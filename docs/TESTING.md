...
| 2 | 🐛 バグ発見: OUTが加算になる | OUT時も increment で加算（要修正） |
| 3 | 🐛 バグ発見: 超過出庫も加算 | 在庫チェックなし＋加算バグ（要修正） |
...
| 6 | 📋 仕様確認: 重複開始を許容する | 二重開始ガードなし（TODO） |
...
| ファイル | 内容 | 優先度 |
|---------|------|--------|
| stock.test.ts | `adjustStock` のOUTが加算になるバグ | 🔴 高 |
| stock.test.ts | 棚卸し重複開始ガードなし | 🟡 中 |
