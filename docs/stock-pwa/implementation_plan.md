...
## 概要
「在庫がいつ、なぜ、どれくらい変わったか」をひと目で確認できるよう、全てのアクション（入荷・出庫・棚卸・手動調整）を一本の時間軸に並べた「在庫台帳」機能を追加します。Rechartsを使用して在庫推移をグラフ化し、不整合の発見を容易にします。

## ユーザー確認事項
- **ライブラリ追加**: 美しいグラフを表示するため、`recharts` ライブラリをインストールします。
- **データ統合**: 以下の3つのテーブルからデータを集約します。
    - `InventoryLog` (棚卸・手動調整)
    - `OrderItem` (入荷)
    - `Transaction` (出庫)

## 変更内容詳細

### 1. サーバーサイド (actions.ts)
#### [NEW] `getProductStockHistory(productId: number)`
- 対象商品の全履歴を取得し、日付順にマージして「その時点での理論在庫」を計算した配列を返します。

### 2. フロントエンド UI
#### [NEW] `/admin/products/[id]/history` ページ
- 商品詳細から遷移できる在庫履歴ページを作成。
- **グラフエリア**: 上部に折れ線グラフで在庫推移を表示。
- **テーブルエリア**: 下部に入荷(+), 出庫(-), 棚卸(Set)の履歴を一覧表示。

## 検証計画
1. 特定の商品（例: KB360）を選択し、履歴が時系列に並んでいるか確認。
2. 入荷や出庫を行った直後、グラフと表が即座に更新されるか確認。
3. グラフ上の在庫数と、実際の在庫管理画面の数字が一致しているか確認。

# 不具合調査・改善: ログイン時の待機時間対策

## 概要
朝一番のログインなど、長時間サーバーがアイドル状態だった後の最初のログインで「ぐるぐる回って進まない」という事象への対策です。

## 原因（推測）
1.  **Next.js開発モードのコンパイル遅延**: サーバーアクションが最初に呼び出される際、コードのコンパイルが走り、数秒〜十数秒かかることがあります。
2.  **Prismaエンジンの起動**: データベース接続に使用するバイナリが停止状態から復帰するのに時間がかかる場合があります。
3.  **Windowsの省電力・アンチウイルス**: 長時間アイドル後のプロセス実行やDBファイルアクセスに対し、OS側でスキャンやスピンアップの待ちが発生することがあります。

## 対策案
-   **UI改善**: 単なる読み込み中アイコンだけでなく、3秒以上かかる場合は「サーバーを準備しています...もう少しお待ちください」といったメッセージを表示し、フリーズではないことを伝えます。
-   **タイムアウト処理**: 万が一完全に止まった場合に備え、一定時間でエラーを出し、再試行を促すようにします。

## 検証
-   サーバー起動直後の初回ログインで、メッセージが表示されるか、その後の処理が正常に進むかを確認。

# 機能追加: 発注単位（基本ロット）への対応

## 概要
発注候補の自動作成時、各商品の「基本発注単位（箱単位など）」に従って発注数を計算するように改善します。

## 実装内容
1.  **モデル変更**:
    -   `Product` モデルに `orderUnit` (Int, default: 1) を追加。
    -   `prisma migrate` または `db push` を実行。

2.  **UI変更**:
    -   `ProductDialog` (商品編集画面) に「発注単位」の入力項目を追加。
    -   `createProduct`, `updateProduct` アクションの引数受け取り処理を修正。

3.  **計算ロジック変更 (`generateDraftOrders`)**:
    -   現状: `不足数 = (最低在庫 - 現在在庫 + 1)`
    -   変更後: `発注数 = Math.ceil(不足数 / 発注単位) * 発注単位`
    -   不足数が0以下の場合は発注しない（変更なし）。

## 検証
1.  商品A（在庫5, 最低在庫10）に対し、発注単位を「12」に設定。
2.  発注候補作成を実行。
3.  発注数が「6」ではなく「12」になっていることを確認。
