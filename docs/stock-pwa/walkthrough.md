...
## カテゴリ別掛率・価格セーフガード・ダッシュボードアラート (2026-03-02)

### 1. カテゴリ別掛率管理
- `CategoryPricingRule`テーブルを新設。カテゴリごとに掛率A/Bを設定し、AUTO商品は`cost×掛率`で自動計算。
- MANUAL商品は掛率対象外で、手動設定を維持。
- 管理画面サイドバーの「材料管理」グループに「価格設定」ページを追加。

### 2. 価格セーフガード
- `cost < priceB < priceA`の順序チェックを違反検知。
- 価格レポートで違反（⚠️赤）・掛率ズレ（△黄）・手動（紫バッジ）を一覧表示。
- 括弧内の数字は「掛率計算値との差額」を意味（AUTO商品のみ表示）。

### 3. ダッシュボードアラート
以下3種の価格アラートをダッシュボードに追加:
- 🔴 **セーフガード違反**: 価格順序が不正な商品（品番と「価格設定へ →」リンク付き）
- 🟡 **掛率ズレ**: AUTO商品で掛率計算値と実値が不一致
- ⚪ **仕入値未設定**: `cost=0`で売値だけ設定されている商品

### 4. 価格レポートのインライン編集
価格設定ページの商品テーブルで、各行の✏️ボタンをクリックすると仕入値・売価A/B・モード(AUTO/MANUAL)をその場で編集・保存可能。

### 5. 発注設定ページの商品設定統合
エアコン発注設定ページ(`/admin/aircon-orders/settings`)に以下を統合:
- **発注単価** / **サフィックス** / **最低在庫数** を1テーブルで一括管理
- ロック1つで誤操作防止、保存時に**変更差分確認ダイアログ**を表示
- サフィックス文字数制限を5→10文字に拡張
- 発注品番プレビュー表示（例: `RAS-AJ22` + `25SWSET` = `RAS-AJ2225SWSET`）
- **エアコン在庫管理ページからサフィックス編集列を削除**（発注設定に一本化）
