...
- [ ] **バグ修正: 代理入力時のデータ消失** <!-- id: 135 -->
    - [ ] `ProxyShopContent` の `clearCart()` 実行条件を修正（更新時に毎回クリアされないようにする） <!-- id: 136 -->
- [ ] **不具合調査・改善: ログイン時の待機時間（コールドスタート）対策** <!-- id: 155 -->
    - [ ] ログイン処理が一定時間以上かかる場合、「準備中...」のメッセージを表示するなどのUX改善 <!-- id: 156 -->
- [ ] **機能追加: 在庫履歴の可視化（時系列台帳）** <!-- id: 150 -->
    - [ ] 全ての在庫変動（棚卸・入荷・出庫・調整）を統合した「時系列データ」の生成アクション作成 <!-- id: 151 -->
    - [ ] 商品ごとの在庫推移を表とグラフ（Recharts）で表示する画面の実装 <!-- id: 152 -->
...
- [x] **緊急修正: 原価保護と在庫反映** <!-- id: 70 -->
    - [x] 商品管理: 原価(priceB)が消えないように保護・ロックする <!-- id: 71 -->
    - [x] 商品管理: 在庫数変更が反映されない問題を修正 <!-- id: 72 -->
    - [x] Kiosk: 出庫完了後の画面遷移時のエラー調査 <!-- id: 73 -->
...
- [ ] **機能追加: 入荷訂正・取消機能** <!-- id: 100 -->
    - [x] `cancelReceipt` (入荷取消) アクションの実装 <!-- id: 101 -->
    - [x] 発注詳細画面への「取消」ボタン追加 <!-- id: 102 -->
    - [ ] 在庫の自動減算（元に戻す）処理の検証 <!-- id: 103 -->
- [x] **機能追加: 発注単位（基本ロット）への対応** <!-- id: 160 -->
    - [x] DBスキーマ変更: `Product.orderUnit` 追加 <!-- id: 161 -->
    - [x] UI変更: 商品編集ダイアログへの入力欄追加 <!-- id: 162 -->
    - [x] ロジック変更: `generateDraftOrders` の計算式修正 <!-- id: 163 -->
    - [x] Import/Exportへの `orderUnit` 対応 <!-- id: 164 -->
    - [x] Exportに `id` 列を追加 <!-- id: 165 -->
    - [x] Import時に `id` があれば優先して商品特定（コード変更対応） <!-- id: 166 -->
    - [x] Import時に `id` があれば優先して商品特定（コード変更対応） <!-- id: 166 -->
    - [x] Import時に `id` があれば優先して商品特定（コード変更対応） <!-- id: 166 -->
    - [x] 修正: Kioskログイン画面の「戻る」ボタンをローディング中無効化 <!-- id: 173 -->
    - [/] スキーマ変更: `Product` に `manufacturer`, `quantityPerBox`, `pricePerBox` を追加 <!-- id: 168 -->
    - [x] ロジック実装: 箱買い/バラ売りの価格・在庫計算（`pricePerBox`対応 - Cart/Store実装） <!-- id: 169 -->
    - [x] UI変更: 管理画面で「メーカー」「箱入数」「箱単価」を設定可能にする <!-- id: 171 -->
    - [x] UI変更: Kioskで「箱」か「バラ」かを選択してカートに入れる機能 <!-- id: 172 -->
