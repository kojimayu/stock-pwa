...
### 🔴 テスト失敗の修正（最優先）
- [ ] `❌ MANUAL: 売価A==売価Bの同額はエラー` テストの修正
  - 原因: `upsertProduct` は `priceMode` を自動判定（L878で`AUTO`リセット→掛率ルール比較で判定）するため、テストで `priceMode:'MANUAL'` を渡しても無視される
  - 修正方針: テスト前にカテゴリ掛率ルールをセットアップし、テスト値が掛率計算と不一致になるように調整 → MANUALと判定される → 同額エラーが出る
