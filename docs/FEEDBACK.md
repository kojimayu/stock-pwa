...
###  最優先（データ損失リスク・業務停止回避）
- ~~[!] **検索ロジックの共通化**~~: ✅ `lib/utils.ts` の `normalizeForSearch` に統一済み（全8ファイルリファクタリング完了）。
...
- [!] **ログインのスピナーが止まらない問題**: PIN入力後スピナーが回り続けて画面が遷移しない。リロードすると正常にログインできる。新規ユーザーではなく既存ユーザーでも発生。原因究明が必要。
