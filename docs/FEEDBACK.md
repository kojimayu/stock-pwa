...
- ~~[!] **エアコン棚卸中のエアコン持出しロック**~~ → ✅ 対応済み（2026-02-24）
- ~~[!] **エアコン棚卸の数量入力に先頭0が入る**~~ → ✅ 対応済み（2026-02-24）
- [!] **棚卸画面のスマホ対応（横スクロール問題）**: スマートフォンで棚卸する際、横スクロールしないと入力欄が表示されず非常に使いづらい。モバイルレスポンシブ対応が必要。
- [!] **同義語・別名検索の対応**: 同じ商品でも呼び方が異なる場合（例: VVF＝VA）やカテゴリ名（例:「電線」）でも検索できるようにエイリアス対応を検討する。
- ~~[ ] **管理番号の重複チェック**~~ → ✅ 対応済み（2026-02-21）注意喚起のみ（ブロックなし）
- [ ] **入荷済みの取り消し機能**: 入荷チェックで誤って入荷確定した場合に取り消しできる仕組み。エアコン・材料両方で必要。
- [ ] **ペアコイル（2分4分）の切り売り対応**: 2分4分ペアコイルのみ切り売り（m単位）に対応する。切り売りの場合は半端が出ることを見越して単価を上げる必要がある。※業者曰くあまり使用しないとのこと（優先度低）。
- [ ] **在庫破棄・調整機能**: 故障や紛失などで在庫を減らす機能がないため、追加が必要。
- ~~[!] **カート内カテゴリ表示**~~ → ✅ 対応済み（2026-02-21）
...
- [!] **業者画面(タブレット)のレイアウト改善**: ~~真ん中の商品選択カラムが狭く、品番が見切れることがある~~ → カテゴリ折りたたみ機能で対応済み。残課題があれば再オープン。
...
- ~~**材料在庫連携**~~ → ✅ 対応済み（2026-02-24）材料⇔エアコン在庫連携実装。
...
- ~~[!] **エアコン持出しのメモ欄追加**~~ → ✅ 対応済み（2026-02-21）
- ~~**発注管理機能**~~ → ✅ エアコン発注は対応済み（PDF生成・メール送信・入荷チェック）。材料発注も基本実装済み。
...
- ~~[ ] **自動ログアウトの担当者記録**~~ → ✅ 対応済み（確認済み: IdleTimerは既にvendorUser情報を渡している）
