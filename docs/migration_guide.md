...
ご指摘の通り、**「データベースファイル(`dev.db`)がアプリケーションフォルダ内にある」** という構成には、運用上のリスクがあります。

### 🚨 注意すべきリスク
1.  **データの先祖返り・分裂（Split Brain）**
    - フォルダごとコピーして「別のPC」で起動すると、その時点から**「別のデータベース」**として独立してしまいます。
    - AのPCで登録したデータは、BのPCには反映されません。
    - 逆に、Bで作業した後にAに戻すと、Bでの作業分は消えてしまいます。

2.  **誤削除のリスク**
    - アプリケーションの更新や再インストールのためにフォルダを削除すると、**中にあるデータベースも一緒に消えます**（バックアップがない場合）。

### ✅ 推奨される運用ルール
1.  **「サーバーは1台」と決める**
    - データベースを持つ「親機」は常に1台に固定してください。
    - 他のPCから使いたい場合は、フォルダをコピーするのではなく、**ブラウザから親機にアクセス**してください。

2.  **定期バックアップ**
    - `dev.db` を定期的に別の場所（NASやクラウドストレージ）にコピーしてください。
    - 付属のバックアップスクリプトもあります（`/save_and_backup`コマンド等）。

3.  **（上級編）データベースの分離**
    - 将来的には、データベースファイルを `C:\AntigravityData\stock.db` のような「アプリとは無関係の場所」に配置し、`.env` でそのパスを指定する構成に変更することをお勧めします。これにより、アプリのフォルダを削除してもデータは残ります。
```
