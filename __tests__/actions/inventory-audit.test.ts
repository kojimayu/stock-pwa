...
describe('getDiscrepancyReport — 差異分析レポート', () => {
    it('✅ 正常: 差異データがない場合は空レポート', async () => {
        ...
        expect(report.summary.totalLoss).toBe(0);
        expect(report.shortageTop.length).toBe(0);
        expect(report.excessTop.length).toBe(0);
        expect(report.reasonBreakdown.length).toBe(0);
    });

    it('✅ 正常: 確定済みの差異がレポートに含まれる', async () => {
        ...
        expect(report.summary.totalLoss).toBe(300); // 3 × 100
        expect(report.shortageTop.length).toBe(1);
        expect(report.shortageTop[0].productName).toBe(p.name);
        expect(report.reasonBreakdown.length).toBe(1);
        expect(report.reasonBreakdown[0].reason).toBe('紛失・不明');
    });

    it('✅ 正常: 過剰データもレポートに反映される', async () => {
        ...
        expect(report.excessTop.length).toBe(1);
    });
