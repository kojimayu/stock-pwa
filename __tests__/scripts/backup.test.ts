...
describe('バックアップ整合性チェック — 自動バックアップの検証', () => {
    ...
    it('✅ WAL整合性: WALモードDBのバックアップにWALファイルが含まれている', () => {
        expect(backup).not.toBeNull();

        // バックアップDBのWALモード確認（PrismaはデフォルトでWALモード）
        const walPath = `${backup!.filePath}-wal`;
        const shmPath = `${backup!.filePath}-shm`;

        // 本番DBのWALファイルサイズを確認
        const sourceWalPath = path.resolve(__dirname, '../../dev.db-wal');
        if (fs.existsSync(sourceWalPath)) {
            const sourceWalSize = fs.statSync(sourceWalPath).size;
            if (sourceWalSize > 0) {
                // 本番WALにデータがある場合、バックアップにもWALが必要
                const backupWalExists = fs.existsSync(walPath);
                console.log(`  本番WAL: ${(sourceWalSize / 1024).toFixed(1)} KB`);
                console.log(`  バックアップWAL: ${backupWalExists ? '存在' : '⚠️ 未存在 → 不完全バックアップの可能性'}`);

                if (!backupWalExists) {
                    // WALなしバックアップは本番DBの最終チェックポイント時点のデータしか含まない
                    console.warn('⚠️ 警告: バックアップにWALファイルがありません。');
                    console.warn('   バックアップは本番DBの最終チェックポイント時点のデータのみです。');
                    console.warn('   backup_db.ps1 を使用するか、3ファイル(.db+.db-wal+.db-shm)でバックアップしてください。');
                }
                expect(backupWalExists).toBe(true);
            } else {
                console.log('  本番WAL: 0 KB（チェックポイント済み、WALなしでOK）');
            }
        } else {
            console.log('  本番WALファイルなし（ジャーナルモードの可能性、WALチェックスキップ）');
        }
    });
});
